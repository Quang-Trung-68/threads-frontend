...
const Modal = NiceModal.create(
  ({
    ...
    const [updatePostApi, { isLoading: isUpdatePostLoading }] =
    ...
    const [reviewApprove, setReviewApprove] = useState(
      requires_reply_approval || false,
    );

    const handleUpdate = async () => {
      if (editedContent.trim() && editedContent.length <= 500) {
        try {
          const updatePromise = updatePostApi({
            postId,
            body: {
          ...
          notifySooner.success(t("post:postUpdated") || t("common:success"));
